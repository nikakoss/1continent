...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:umi="http://www.umi-cms.ru/TR/umi">
    ...
    <xsl:template match="result[@module = 'catalog' and @method = 'object'][page/@type-id != '136']">
        ...
        <xsl:variable name="district" select=".//property[@name = 'rajon_k']/value/item/@name"/>
        ...
        <div class="wrapper">
            ...
            <div class="content_detail" itemscope='"' itemtype="http://schema.org/Product">
                ...
                <div class="catalog">
                    <h2>Квартиры в <xsl:value-of select="$title"/></h2>
                    <xsl:apply-templates select="document(concat('udata://content/menu/0/0/', $id))/udata/items" mode="kvartiri_group"/>
                </div>
    ...
    <!--===========================================================================================================================-->
    <!-- Квартиры -->
    <xsl:template match="items" mode="kvartiri_group">
        <xsl:apply-templates select="item" mode="kvartiri_group"/>
    </xsl:template>
    <xsl:template match="item" mode="kvartiri_group">
        <div class="apartments_group">
            <div class="a_g_name">
                <!--a href="{@link}">
                <xsl:value-of select="@name"/>
            </a-->
            ...
            <xsl:apply-templates select="document(concat('udata://content/menu/0/0/', @id))/udata/items" mode="kvartiri_test"/>
        </div>

    </xsl:template>
    <xsl:template match="items" mode="kvartiri">
        <xsl:apply-templates select="item" mode="kvartiri"/>
    </xsl:template>
    <xsl:template match="items" mode="kvartiri_test">
        <xsl:apply-templates select="item" mode="kvartiri_jk"/>
    </xsl:template>

    <xsl:template match="item" mode="kvartiri">
            ...
            <div class="a_l">
                <a href="{$link}">
                    <img alt="{$title}" title="{$title}">
                        <xsl:attribute name="src">
                            <xsl:value-of select="document(concat('udata://system/makeThumbnailFull/(.', $photo, ')/100/100/default/0/1/5/80'))//src"/>
                        </xsl:attribute>
                    </img>
                </a>
            </div>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th class="a_r_i_price"> Стоимость </th>
                            </tr>
                        </thead>
                        <tr>
                            <td class="a_r_i_op">
                            ...
                            <td class="a_r_i_price">
                                <xsl:value-of select="format-number($cena, '### ###', 'european')"/> руб. </td>
                        </tr>
                    </table>
                </div>
            </div>
    ...
    <xsl:template match="item|page " mode="kvartiri_jk">
