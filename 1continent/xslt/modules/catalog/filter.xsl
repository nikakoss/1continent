...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:umi="http://www.umi-cms.ru/TR/umi">
    ...
    <xsl:template match="udata[@module = 'catalog' and @method = 'search']" mode="filter">
        <form action=" /{$current_catalog}" method="get" onsubmit="yaCounter21668962.reachGoal('sbmt_filter'); return true;">
            <xsl:apply-templates select="document('udata://catalog/getCategoryList/notemplate/0/0/1')/udata/items" mode="filter_cat"/>
    ...
    <xsl:template match="items" mode="filter_cat">
        <div class="f_cat">
            ...
            <xsl:apply-templates select="item[@id != '393']" mode="filter_cat"/>
        </div>
    </xsl:template>

    <xsl:template match="item[@id != '393']" mode="filter_cat">
        <a href="{@link}">
            <xsl:value-of select="."/>
        </a>
    </xsl:template>
