	...
	<xsl:template match="purchasing[@stage = 'payment'][@step = 'bonus']">
		...
		<form id="bonus_payment" method="post" action="do/">
			...
			<div style="margin:0 15px;">
				<p>Вы можете оплатить Ваш заказ накопленными бонусами.</p>
				<xsl:text>Заказано товаров на сумму </xsl:text>
				...
				<xsl:text> Доступно бонусов на сумму </xsl:text>
				<xsl:value-of select="$currency-prefix" />
				<xsl:text> </xsl:text>
			...
			<div><label>Оплатить бонусами <xsl:value-of select="$currency-prefix" /><input type="text" name="bonus" /><xsl:value-of select="$currency-suffix" /></label></div>
